...
## Overview
After a PO is Created, the requester or Procurement team records that goods or services have been received against that PO. Receipts can be partial or full. Each confirmed GR sends an accounting transaction to Bookkeeping. GR records are immutable — corrections are handled through a GR Reversal document.
...
GR is always recorded as a **percentage of the line item**, regardless of item type (Asset or Expense). The percentage is cumulative — each GR adds to the previously received percentage until the item reaches 100%.
...
| Prev. Received | Cumulative % already confirmed in prior GRs (pill: None / n% received / 100% received) |
| Remaining | 100% minus Prev. Received % |
| % to Receive | Numeric input (e.g. enter `60` for 60%). Disabled (shows —) if Remaining = 0%. Cannot exceed Remaining %. |

Fully received items (100%) are visually dimmed and the input is disabled.

**Live footer totals** (appear once any % > 0 is entered):
- Items Selected count
- Avg. % Received (average of entered percentages across selected items)
- Est. Amount (sum of `entered % ÷ 100 × Line Total` per item)
...
If a GR was recorded incorrectly, the user can cancel it by clicking the **bin icon** on a Confirmed GR row in the list. The system:
1. Opens a Reversal modal showing the original GR details (GR number, PO, vendor, amount, date) and the reversal document number (e.g. GR-0001-R)
2. Requires a mandatory cancellation reason (minimum 5 characters) before the Confirm button enables
3. Creates a GR Reversal document with negative quantities and amounts, dated today
4. Posts an offsetting accounting transaction to Bookkeeping on the reversal date
...
The original GR is tagged **Reversed**. The reversal document is tagged **Reversal**. The bin icon is only available on Confirmed GRs — Reversed and Reversal documents cannot be cancelled again.
...
| GR Date | GR Number | PO Number | Vendor | Items | Total Amount | Status | |
|---|---|---|---|---|---|---|---|
| 2026-06-01 | GR-0001 | PO-0042 | ABC Co. | 3 items | 145,000 | Reversed | ▶ |
| 2026-06-03 | GR-0002 | PO-0038 | XYZ Co. | 1 item | 12,500 | Confirmed | ▶ |
| 2026-06-05 | GR-0001-R | PO-0042 | ABC Co. | 3 items | -145,000 | Reversal | ▶ |
...
Bin icon (🗑) appears only on Confirmed rows in this view.
...
- **Items table:** Shows all PO line items with line total value, previously received %, remaining %, and a % to receive input. Fully received items (100%) are disabled.
- **Percentage input:** User enters a number (e.g. 60 for 60%). Applies to all item types — Asset and Expense alike. Cannot exceed Remaining %. Error state shown inline; Confirm button stays disabled until resolved.
- **Partial receipt:** User can record a % less than remaining. System tracks cumulative received %. PO GR Status updates (Not Started / Partially Received / Fully Received).
...
- **GR Reversal:** Triggered by the bin icon on a Confirmed GR. System opens reversal modal, requires mandatory reason (≥5 characters), creates GR Reversal with negative quantities and amounts, posts offsetting Bookkeeping transaction dated the reversal date.
...
- **Bin icon availability:** Bin icon appears only on Confirmed GRs. Reversed and Reversal documents cannot be cancelled again.
...
- **Document View:** Default view. One row per GR, expandable to see line items. Bin icon on Confirmed rows.
...
## Open Questions
- [ ] **GR form — received date:** Should the date default to today and be editable, or always auto-set to today?
